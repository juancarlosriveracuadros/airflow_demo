...
def _build_features(raw_df: DataFrame, model_dir: str) -> DataFrame:
    ...
    #    key=minio.login,
    #    secret=minio.password,
    #    client_kwargs={'endpoint_url': minio.extra_dejson['endpoint_url']},
    #    use_ssl=False
    #)
    ...
    #    object_name=f"{model_dir}/scalar.joblib",
    #    data=BytesIO([metrics_df, scaler]),
    #    length=len(data)
    #)
...
def _predict_housing(feature_df: DataFrame, model_file_uri: str) -> DataFrame:
    ...
    #loaded_model = load(model_file_uri)
